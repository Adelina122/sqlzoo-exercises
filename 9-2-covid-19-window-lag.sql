...
SELECT name, DATE_FORMAT(whn,'%Y-%m-%d') AS date, 
  confirmed - LAG(confirmed,1) OVER (PARTITION BY name ORDER BY date) AS new
FROM covid
WHERE name = 'Italy'
  AND WEEKDAY(whn) = 0 
  AND YEAR(whn) = 2020
...
SELECT 
  world.name,
  ROUND(100000*confirmed/population,2) AS 'infection rate',
