...
*/
SELECT COUNT(DISTINCT id) AS stops
FROM stops
...
*/
SELECT a.num, a.company, stop.name, d.num, d.company
FROM route a 
  JOIN route b 
    ON (a.company=b.company AND a.num=b.num)
  JOIN stops stop ON (b.stop=stop.id)
  JOIN route c ON (stop.id=c.stop)
...
WHERE a.stop = 
  ...
  AND d.stop = 
    ...
    FROM stops 
    WHERE name = 'Lochend')
ORDER BY a.num, stop.name, d.num
