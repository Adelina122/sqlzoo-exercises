/*
Table:
nns(ukprn, institution, subject, level, question, A_STRONGLY_DISAGREE, A_DISAGREE, A_NEUTRAL,
A_AGREE, A_STRONGLY_AGREE, A_NA, CI_MIN, score, CI_MAX, response, sample, aggregate)
*/
...
*/
SELECT subject, SUM(response) AS responded
FROM nss
WHERE question='Q22'
...
'(8) Computer Science' and '(H) Creative Arts and Design'.
*/
SELECT subject, SUM(response*A_STRONGLY_AGREE/100) AS 'strongly agree total'
FROM nss
WHERE question='Q22'
AND subject IN ('(8) Computer Science', '(H) Creative Arts and Design')
GROUP BY 1
...
*/
SELECT subject, ROUND(SUM(response* A_STRONGLY_AGREE)/SUM(response), 0) AS 'strongly agree percentage'
FROM nss
WHERE question='Q22'
AND subject IN ('(8) Computer Science', '(H) Creative Arts and Design')
GROUP BY 1
...
*/
SELECT institution,ROUND(SUM(score*response)/SUM(response), 0) AS 'average score'
FROM nss
WHERE question='Q22'
...
*/
SELECT institution, SUM(sample) AS 'total sample size', SUM(CASE WHEN subject='(8) Computer Science' THEN sample ELSE 0 END) AS 'computing students' 
FROM nss
WHERE question='Q01'
AND (institution LIKE '%Manchester%')
GROUP BY 1
